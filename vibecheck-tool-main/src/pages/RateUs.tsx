...
/** Returns true when the id looks like a proper UUID (real Supabase user). */
...
const RateUs = () => {
  ...
  // Holds reviews submitted during this session for demo users.
  const [localReviews, setLocalReviews] = useState<SiteReview[]>([]);
  ...
  // Local session reviews first, then real DB reviews, then seed data.
  const reviews = [...localReviews, ...dbReviews, ...SEED_REVIEWS];

  const submitMutation = useMutation({
    mutationFn: async ({ rating, text }: { rating: number; text: string }) => {
      if (!user) throw new Error("Not signed in");

      // Mock users (demo accounts) don't have real UUIDs, so skip the DB write.
      if (!isUUID(user.id)) {
        return {
          id: `local-${Date.now()}`,
          user_id: user.id,
          ...
          profiles: { full_name: profile?.full_name ?? "You", avatar_url: null },
    ...
    onSuccess: (newReview) => {
      if (newReview) {
        // Demo mode: add the review to local display state immediately.
