  ...
  return (
    <nav className="sticky top-0 z-50 backdrop-blur-sm border-b py-0 bg-background/95 border-border transition-colors duration-500">
      ...
      <div className="container mx-auto px-6 flex items-center justify-between py-4">
        <Link to="/" className="flex items-center gap-2.5 font-display text-2xl text-foreground tracking-tight">
          {/* Artevia logo mark — diamond frame with centred dot */}
          <svg
            viewBox="0 0 32 32"
            width="26"
            height="26"
          ...
          >
            <rect
              x="8" y="8" width="16" height="16" rx="1.5"
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2">
                ...
                {profile?.twoFactorEnabled && (
                  <ShieldCheck size={13} className="text-primary" title="2FA enabled" />
                )}
              </div>
              <Link to="/account" title="Account settings" className="text-muted-foreground hover:text-primary transition-colors">
                <ShieldCheck size={16} />
              </Link>
          ...
          {user ? (
            <>
              <Link to="/account" onClick={() => setOpen(false)}
                className="block px-3 py-2 text-sm tracking-wider uppercase text-foreground">
                Account
              </Link>
